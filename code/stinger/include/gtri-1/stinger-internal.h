...
#define STINGER_FORALL_EB_BEGIN(STINGER_,STINGER_SRCVTX_,STINGER_EBNM_)	\
  do {									\
    ...
    if (stinger_srcvtx__ >= 0) {					\
      ...
      while (stinger_eb__ != STINGER_->ebpool) {						\
...
#define STINGER_FORALL_EB_MODIFY_END(STINGER_)					\
	} while (0);							\
	stinger_eb__ = STINGER_->ebpool + stinger_eb__->next;				\
      }									\
    }									\
  } while (0)
...
struct stinger_vb
{
  ...
  int64_t physID;      /**< A byte-string of physical ID */
  uint8_t physIDlength; /**< The length of the physical ID */
...
struct curs etype_begin (struct stinger * S, struct stinger_vb *v, int etype);
