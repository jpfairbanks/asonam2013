...
get_from_ebpool (struct stinger * S, eb_index_t *out, size_t k)
{
  eb_index_t ebt0;
  {
    ebt0 = stinger_size_fetch_add (&S->ebpool->ebpool_tail, k);
...
stinger_typed_outdegree (const struct stinger * S, int64_t i, int64_t type) {
  int64_t out = 0;
  struct stinger_eb *current_eb = S->ebpool->ebpool + (S)->LVA[(i)].edges;
  while(current_eb != S->ebpool->ebpool) {
    if(current_eb->etype == type) {
      out += current_eb->numEdges;
    }
    current_eb = S->ebpool->ebpool + current_eb->next;
  }
  return out;
}
...
{
  return S_->ebpool->ebpool + S_->LVA[i_].edges;
}
...
stinger_consistency_check (struct stinger *S, uint64_t NV)
{
  ...
  for (uint64_t i = 0; i < NV; i++) {
    ...
    while (curBlock != S->ebpool->ebpool) {
      ...
      curBlock = S->ebpool->ebpool + curBlock->next;
    }
...
struct stinger *stinger_new (void)
{
  ...
  G->physMapBuffer = xmalloc (STINGER_MAX_LVERTICES * 128 * sizeof(char));
...
MTA ("mta inline")
struct curs
etype_begin (struct stinger * S, struct stinger_vb *v, int etype)
{
  struct curs out;
  assert (v);
                     ...
                     int64_t weight, int64_t timestamp)
{
  ...
  curs = etype_begin (G, &G->LVA[from], type);
  /*
  Possibilities:
...
/** @brief OR weight of a directed edge.
 *
 *  Atomically ORs the weight of a typed, directed edge.
 *  Recent timestamp is updated.
 *
 *  @param G The STINGER data structure
 *  @param type Edge type
...
MTA ("mta inline")
int
stinger_OR_edge (struct stinger *G,
                   int64_t type, int64_t from, int64_t to,
                   int64_t weight, int64_t timestamp)
{
  if(from == to)
...
/** @brief OR an undirected edge.
 *
 *  ORs the weight of a typed, undirected edge.
 *  Recent timestamp is updated.
 *
 *  @param G The STINGER data structure
 *  @param type Edge type
...
MTA ("mta inline")
int
stinger_OR_edge_pair (struct stinger *G,
                        int64_t type, int64_t from, int64_t to,
                        int64_t weight, int64_t timestamp)
{
  STINGERASSERTS();

  int rtn = stinger_OR_edge (G, type, from, to, weight, timestamp);
  int rtn2 = stinger_OR_edge (G, type, to, from, weight, timestamp);
                           ...
                           /* if !ts or !first_ts */ )
{
  ...
  MTA ("mta assert parallel")
	...
	G->ebpool->ebpool[block[blkoff[v+1]-1]].next = LVA[from].edges;
...
stinger_save_to_file (struct stinger * S, uint64_t maxVtx, const char * stingerfile)
{
  ...
  fwrite(&endian_check, sizeof(int64_t), 1, fp);
  fwrite(&maxVtx, sizeof(int64_t), 1, fp);
  fwrite(&etypes, sizeof(int64_t), 1, fp);
  fwrite(type_offsets, sizeof(int64_t), etypes+1, fp);
  fwrite(offsets, sizeof(int64_t), (maxVtx+2) * etypes, fp);
  fwrite(ind, sizeof(int64_t), 4 * type_offsets[etypes], fp);
  fclose(fp);
...
stinger_open_from_file (const char * stingerfile, struct stinger ** S, uint64_t * maxVtx)
{
  ...
  *S = stinger_new();
  ...
  fread(&local_endian, sizeof(int64_t), 1, fp);
  int64_t etypes = 0;
  fread(maxVtx, sizeof(int64_t), 1, fp);
  fread(&etypes, sizeof(int64_t), 1, fp);
  ...
  fread(type_offsets, sizeof(int64_t), etypes+1, fp);
  fread(offsets, sizeof(int64_t), (*maxVtx+2) * etypes, fp);
  ...
  fread(ind, sizeof(int64_t), 4 * type_offsets[etypes], fp);
